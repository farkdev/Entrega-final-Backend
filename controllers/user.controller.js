...
class UserController {
    ...
    resetPass = async(req, res) => {
        try {
            ...
            logger.error(error);
            res.status(500).send({ status: 'error', message: 'Error al cambiar la contraseña' });
        }
    }
    ...
    getAllUsers = async(req, res) =>{
        try {            
            ...
            const result = allUsers.map(user=> new UserDto(user))
    ...
    deleteUsers = async(req, res) =>{
        try {
            ...
            if (inactiveUsers.length === 0) {
                return res.status(200).send({ status: 'success', message: 'No hay usuarios para eliminar'});
            }
            ...
            res.status(200).send({ status: 'success', message: 'Usuarios eliminados correctamente' });
